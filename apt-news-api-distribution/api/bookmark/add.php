	...
	try{
		...
		class Signup{
			...
			public function DoSignup(){
				...
				if($this->IsError == false){
                    $bookmark ='bookmark_'.rand_string(3).time().rand_string(3);
                    $query = "INSERT INTO bookmark_items (uid,rid,user_id,rdate,udate) VALUES ";
                    $query .= "(:uid,:rid,:user_id,:rdate,:udate)";
                    $query .= " ON DUPLICATE KEY UPDATE ucount = ucount + 1 ";
                    $stmt = $SystemDbConn->prepare($query);
                    $stmt->bindParam(":uid",$bookmark,PDO::PARAM_STR);
					...
					if($stmt->execute()){
					    if($stmt->rowCount() > 0){
					        $this->IsError = false;
					        $this->response = ['status'=>true,'data'=>"Bookmark add successfully",'code'=>200];    
					    }else{
					        $this->IsError = true;
