		...
		class Signup{
			...
			public function DoSignup(){
				...
				if($this->IsError == false){
                    ...
                    $query .= " FROM bookmark_items INNER JOIN news ON (news.uid = bookmark_items.rid) INNER JOIN category_linking ON(category_linking.rid = news.uid) LEFT JOIN rating_items ON (rating_items.rid = news.uid)  WHERE bookmark_items.user_id = :user_id and news.status = 'active' ";
                    $query .= " ORDER BY bookmark_items.rdate DESC ";
                    ...
                    if($stmt->execute()){
                        if($stmt->rowCount() > 0){
                            ...
                            $this->IsError = true;
                            return ['status'=>false,'data'=>'No bookmark found','totalrows'=>0,"code"=>404];
                        }
                    }else{
                        $this->IsError = true;
                ...
                if($this->IsError == false){
                    ...
                    if($stmt->execute()){
                        if($stmt->rowCount() > 0){
                            ...
                            $this->IsError = true;
                            $this->response =  ['status'=>false,'data'=>'No bookmark found','totalrows'=>0,"code"=>404];
                        }
                    }
                    else{
                        $this->IsError = true;
