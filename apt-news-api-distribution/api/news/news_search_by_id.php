		...
		class Signup{
			...
			public function DoSignup(){
				...
				if($this->IsError == false){
                    ...
                    $query .= " FROM news  ";
                    $query .= " INNER JOIN category_linking ON(category_linking.rid = news.uid) ";
                    
                    $query .= " LEFT JOIN bookmark_items ON (news.uid = bookmark_items.rid and bookmark_items.user_id = :user_id)  LEFT JOIN rating_items ON (rating_items.rid = news.uid and rating_items.user_id = :user_id)  WHERE news.status = 'active' and news.uid = :News_id ";
                    ...
                    if($stmt->execute()){
                        ...
                        $this->IsError = true;
                        $this->response =  ['status'=>false,'data'=>"Something went wrong",'debug'=>$stmt->errorInfo(),"code"=>400];
                    }
                ...
                if($this->IsError == false){
                    $this->category = $this->result_array['category'];
                    $this->newsid = $this->result_array['uid'];
                    $query = "SELECT bookmark_items.uid as bookmark_id,bookmark_items.rid,news.*,category_linking.category,rating_items.uid as rating_id,rating_items.rating,(SELECT count(id) FROM rating_items WHERE rid = news.uid) as total_like  ";
                    $query .= " FROM news  ";
                    $query .= " INNER JOIN category_linking ON(category_linking.rid = news.uid and category_linking.category =:category) ";
                    $query .= " LEFT JOIN bookmark_items ON (news.uid = bookmark_items.rid and bookmark_items.user_id = :user_id)  LEFT JOIN rating_items ON (rating_items.rid = news.uid and rating_items.user_id = :user_id)  WHERE news.status = 'active' and news.uid != :newsid LIMIT 3 OFFSET 0";
                    $stmt = $SystemDbConn->prepare($query);
                    $stmt->bindParam(':user_id',$this->user_id,PDO::PARAM_STR);
                    $stmt->bindParam(':category',$this->category,PDO::PARAM_STR);  
                    $stmt->bindParam(':newsid',$this->newsid,PDO::PARAM_STR);  
                    if($stmt->execute()){
                        if($stmt->rowCount() > 0){
                            $result = $stmt->fetchAll();
                            
                            $this->category_result_array = array();
                            foreach($result as $key=>$value){
                                $result_array_tmp = array();
                                $result_array = $value;
                                ...
                                $this->category_result_array[] = $result_array_tmp;
                            }
                        }
                    }
                ...
                if($this->IsError == false){
                    $this->response = ['status'=>true,'data'=>$this->result_array,'related'=>$this->category_result_array,"code"=>200];
                }
