<?php
    ...
    //             }
				...
				if($this->IsError == false){
                    $this->status = "active";
                    $query = "DELETE  FROM rating_items  WHERE (user_id=:user_id and rid =:news_id)";
                    $stmt = $SystemDbConn->prepare($query);
					...
					if($stmt->execute()){
					    if($stmt->rowCount() > 0){
    					    $this->IsError = false;
    					    $this->response = ['status'=>true,'data'=>"Rating removed successfully",'code'=>200];
					    }else{
					        $this->IsError = true;
                            $this->response = ['status'=>false,'data'=>'Rating removed successfully','code'=>400,'EVersion'=>'E.2.0'];   
					    }
					}else{
						$this->IsError = true;
