	...
	try{
		...
		class Signup{
			...
			public function DoSignup(){
				...
				if($this->IsError == false){
                    $query = "SELECT rating_items.uid as rating_id,rating_items.rid,rating_items.rating as rating,news.*,category_linking.category,bookmark_items.uid as bookmark_id,(SELECT count(id) FROM rating_items WHERE rid = news.uid) as total_like    ";
                    $query .= " FROM rating_items INNER JOIN news ON (news.uid = rating_items.rid) INNER JOIN category_linking ON(category_linking.rid = news.uid) LEFT JOIN bookmark_items ON (bookmark_items.rid = news.uid)  WHERE rating_items.user_id = :user_id and news.status = 'active' ";
                    $query .= " ORDER BY rating_items.rdate DESC ";
                    ...
                    if($stmt->execute()){
                        if($stmt->rowCount() > 0){
                            ...
                            $this->IsError = true;
                            return ['status'=>false,'data'=>'No rating found','totalrows'=>0,"code"=>404];
                        }
                    }else{
                        $this->IsError = true;
                ...
                if($this->IsError == false){
                    ...
                    if($stmt->execute()){
                        if($stmt->rowCount() > 0){
                            ...
                            $this->IsError = true;
                            $this->response =  ['status'=>false,'data'=>'No rating found','totalrows'=>0,"code"=>404];
                        }
                    }
                    else{
                        $this->IsError = true;
