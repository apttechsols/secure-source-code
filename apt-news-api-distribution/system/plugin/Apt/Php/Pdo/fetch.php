<?php
	function AptPdoFetch($Data=array()){
		$DefaultCheckFor = 'All'; $DefaultCheckType = 'Equal'; $AcceptNullCondtion = False; $FetchRowNo = 1;
		foreach ($Data as $key=>$value){
			${ $key } = $value;
		}

		if(!array_key_exists("DbCon",$Data) or !array_key_exists("TbName",$Data) or $DbCon == '' or $TbName == ''  or $DefaultCheckFor == '' or $DefaultCheckType == '' or $FetchRowNo == '' or ($AcceptNullCondtion != False and $AcceptNullCondtion != true)){
		...
		if($FetchData != ''){
			...
			$FetchDataArray = explode("::::",$FetchData);
			$i = 0;
			
			foreach ($FetchDataArray as $value) {
				$i++;
					
				if(preg_replace("/[^A-Za-z0-9_,:]/","",$value) != "" && preg_replace("/[^A-Za-z0-9_,:]/","",$value) == $value){
					$TmpArray = explode('::,::',$value);
					$TmpFetchDataFormat = $value." AS ".$value;
					if($TmpArray[1] != ''){
						if(strtolower($TmpArray[1]) == 'nenc'){
							$TmpFetchDataFormat = $TmpArray[0];
						}
					}
					$StmtFetchDataString .= ", ".$TmpFetchDataFormat;
				}else{
					return ["status"=>"Error","msg"=>"Invalid Data Fatch column name detect","code"=>400];
				}
			}
			$StmtFetchDataString = trim($StmtFetchDataString,', ');
		}else{
			$StmtFetchDataString = Null;
		}
		...
		if($FetchData == ''){
			$StmtString = "SELECT NULL FROM $TbName WHERE $CondtionString";
		}else{
			$StmtString = "SELECT $StmtFetchDataString FROM $TbName WHERE $CondtionString";
		}
		
		if($FormatDataOrder != ''){ $StmtString .= $FormatDataOrder; }

		if($Limit != ''){ $StmtString .= " LIMIT $Limit"; }
		if($OFFSET != ''){ $StmtString .= " OFFSET $OFFSET"; }
		...
		if($stmt->execute()){
			if($stmt->rowCount() > 0){
				...
				return ['status'=>'Error','msg'=>'Given Data not found','totalrows'=>0,'reason'=>json_encode($stmt->errorinfo()),"code"=>404];
			}
		}else{
